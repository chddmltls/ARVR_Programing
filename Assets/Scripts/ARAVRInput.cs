#define PC
//#define Oculus //주석을 제거하면 피씨와 오큘러스 선택 가능
...
{
#if PC
    public enum ButtonTarget //enum -> 값들의 묶음
    {
        Fire1,
        Fire2,
        Fire3,
...
#elif Oculus
        One = OVRInput.Button.One, //OVR INput -> Meta SDK에서 정의한 기능
...
#endif
#if Oculus
    static Transform GetTransform() //Get <- 값을 읽어옴 (접근자) <-> 대응으로 Set이 있음. 값을 바로 넣음
    {
...
# endif
    ...
    // 왼쪽 컨트롤러
    static Transform lHand; //외부에서 변경할수 없는 값
    // 씬에 등록된 왼쪽 컨트롤러를 찾아 반환
    public static Transform LHand //사전에 설정된 값이 변형되지 않도록 lHand와 LHand를 구분
    {
        get
        {
            if (lHand == null)
    ...
    public static Vector3 LHandPosition
    {
        get
        {
            ...
            pos = GetTransform().TransformPoint(pos); //TransformPoint로 VR이 위치값을 읽을 수 있게 바꿔줌
            return pos;
#endif
        }
    }
